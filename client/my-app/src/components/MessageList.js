...
export default function MessageList({
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      {/* Header - Fixed height sama dengan ChatArea */}
<div className="flex-shrink-0 px-4 border-b border-gray-200 h-[64px] md:h-[80px] flex flex-col justify-center">
  <h2 className="text-xl md:text-2xl font-black text-slate-900 mb-0.5">Messages</h2>
  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
    {filteredConversations.length} Conversations
  </p>
</div>

{/* Search Bar - Kurangi vertical padding */}
<div className="px-4 py-3 md:px-6">
  <div className="relative">
    <input
      ...
      className="w-full pl-9 pr-4 py-2 md:py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-300 focus:border-slate-300 transition-all"
    />
    {/* Search Icon */}
    <svg
...
{/* Tabs - Kurangi padding */}
<div className="px-4 md:px-6 py-2 flex gap-4 md:gap-6 border-b border-slate-200">
  {tabs.map((tab) => (
    <button
      key={tab}
      onClick={() => setActiveTab(tab)}
      className={`text-[10px] md:text-[11px] font-black uppercase tracking-wider pb-2 md:pb-3 transition-all relative ${
        activeTab === tab
          ? "text-slate-900"
...
{/* Conversation List - Optimasi spacing */}
